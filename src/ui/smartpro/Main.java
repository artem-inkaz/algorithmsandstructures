...
import ui.smartpro.notebook.Notebook;
import ui.smartpro.notebook.SortNotebooksSelection;
...
/*        способ 3 - сортировка быстрая
    ...
    private static void displayNotebooks(List<Notebook> list) {
        for (Notebook n : list) {
            System.out.printf("Ноутбук:\tс\\н: %s,\tцена: %s,\tпамять: %s,\tпроизводитель: %s%n", n.getId(),
                    n.getPrice(),
                    n.getMemory(), n.getModel());
        }
    }
    ...
    private static Notebook createNotebook() {
        ...
        return new Notebook(producer,price, memory);
    }
